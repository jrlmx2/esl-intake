...
export class Day {
    ...
    constructor(
        ...
        this.during =
            semesterStart.isSame(date) ||
            semesterEnd.isSame(date) ||
            (semesterStart.isBefore(date) && semesterEnd.isAfter(date))
...
export class Month implements Iterable<CalendarRow> {
    ...
    static from(date: Dayjs): Month {
        ...
        if (Month.singleton.has(key)) return Month.singleton.get(key)!
    ...
    is(date: Dayjs): boolean {
        return (
            date.format('MMM') === this.month &&
            date.format('YYYY') === this.year
        )
    }
    ...
    [Symbol.iterator](): Iterator<CalendarRow & Symboler> {
        ...
        return {
            next: (): IteratorResult<CalendarRow & Symboler> => {
                ...
                counter += 1
                if (!week) return { done: true, value: null }
                else return { value: week.flatten(), done }
            }
        }
    }
    ...
    private getWeek(date: Dayjs): Week {
        ...
        if (date.day() !== 0) week = this.weeks.at(-1)
...
export class Calendar implements Iterable<CalendarRow> {
    ...
    constructor(semesterObj: Semester) {
        ...
        this.id = Symbol.for(semesterObj.semester)
    }
    ...
    static from = (semester: Semester): Calendar => {
        ...
        let calendarRow: CalendarRow & Symboler =
            calendarRowCreator(workingDate)
        let day: Day
        while (workingDate.isBefore(calendarEnd)) {
            ...
            day = new Day(
                ...
                calendarStart.isSame(workingDate),
                events
            )
